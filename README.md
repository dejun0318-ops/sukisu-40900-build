...
The build process follows [ShirkNeko/GKI_KernelSU_SUSFS](https://github.com/ShirkNeko/GKI_KernelSU_SUSFS) and the Android 14 / 6.1 SUSFS compatibility fix from [zzh20188/GKI_KernelSU_SUSFS](https://github.com/zzh20188/GKI_KernelSU_SUSFS). It pins the SukiSU builtin, SUSFS, and AnyKernel3 commits. Google deleted the `2025-06` monthly branch, so the workflow uses its preserved `android14-6.1-2025-06_r11` release tag. Its GKI commit `6ab8c9a86a33` matches the hash in the device's kernel version. The workflow checks the full commit and `6.1.138` before compilation. `BUILD_INFO.txt` records the exact GKI commit.
